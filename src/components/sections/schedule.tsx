        ...
        <Tabs defaultValue={schedule[0].day}>
          ...
          {schedule.map((d) => (
            <TabsContent key={d.day} value={d.day}>
              <div className="mb-6 flex items-center justify-between font-mono text-[17px] uppercase tracking-[0.15em] text-slate-600">
                <span>{d.day}</span>
                <span>Moderator &middot; {d.moderator}</span>
              </div>
              <ol className="glass-card divide-y divide-black/8 rounded-2xl">
                {d.items.map((item, i) => (
                  <motion.li
                  ...
                  >
                    <span className="flex items-center gap-2 font-mono text-[17px] text-green-600 sm:w-40 sm:shrink-0">
                      <Clock className="h-3.5 w-3.5" />
                      {item.time}
                    </span>
                    <span className="font-display text-[21px] text-ink-800">
                      {item.activity}
                    </span>
                    {item.person && (
                      <span className="flex items-center gap-2 text-[17px] text-slate-600 sm:ml-auto">
